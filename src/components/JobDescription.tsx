...
import { JobDescriptionProps } from "../modules/modules";

export function JobDescription({description}: JobDescriptionProps):JSX.Element {
    const postDate = description?.createdAt.slice(0, 10);

    return (
        <>
            <div className="flex mt-[56px] justify-between  border-b-2 border-[#3A4562]">
                <h2 className="text-[28px] font-bold">Jobs Details</h2>
            ...
            <div className="flex justify-between mb-[7px]">
                <div className="w-[400px]">
                    <p className="text-[24px] font-bold">{description?.title}</p>
                </div>
                <div className="flex flex-col">
                    <p className="text-[20px] font-bold">$ {description?.salary}</p>
            ...
            <div className="flex flex-col">
                ...
                <p className="text-[18px] mb-[7px]">{description?.description}</p>
                <span className="font-bold text-[18px]">Benefits:</span>
                {description?.benefits.map((el:string,index:number) => <li key={index} className="text-[18px]">{el}</li>)}
            </div>
            <ApplyButton />
        </>
    );
