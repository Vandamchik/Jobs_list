...
export function JobDescription({descriptionInfo}: JobDescriptionProps):JSX.Element {
    const {createdAt, title, salary, description, benefits} = descriptionInfo
    const postDate = createdAt.slice(0, 10);
    ...
    return (
        <>
            ...
            <div className="flex flex-col">
                ...
                <p className="text-[18px] mb-[7px]">{description}</p>
                <span className="font-bold text-[18px]">Benefits:</span>
                {benefits.map((el:string,index:number) => <li key={index} className="text-[18px]">{el}</li>)}
            </div>
